...
export const UserController = {
    async createUser(c: Context) {
        try {
            const userData = await c.req.json() as Pick<User, 'email' | 'username' | 'name' | 'lastName'>
            const { email, username, name, lastName } = userData

            if (!email || !username || !name || !lastName)
                return fail(c, Errors['ValidationError'], 400)

            if(email.split('@').length !== 2) return fail(c, 'InvalidEmail', 400)

            if (await UserModel.getUserByUsername(username))
                return fail(c, Errors['UsernameAlreadyTaken'], 409)

            if (await UserModel.getUserByEmail(email))
                return fail(c, Errors['EmailAlreadyInUse'], 409)

            const user = await UserModel.createUser({...userData, password: generateRandomPassword(10)})
            return ok(c, omitPassword(user), 201)

        } catch {
            return fail(c, Errors['ServerError'], 500)
        }
    },
    ...
    async editUser(c: Context) {
        try {
            ...
            if (!await UserModel.getUserById(userId))
                return fail(c, Errors['UserNotFound'], 404)
    ...
    async getUser(c: Context) {
        try {
            ...
            return user ? ok(c, omitPassword(user)) : fail(c, Errors['UserNotFound'], 404)
